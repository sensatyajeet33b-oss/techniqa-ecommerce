    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords: { latitude: lat, longitude: lon } }) => {
        try {
          ...
          setShip(s => ({
            ...s,
            address: [a.road, a.neighbourhood, a.suburb].filter(Boolean).join(", ") || s.address,
    ...
    return (
      <div className="co-panel">
        ...
        <div className="co-form">
          {/* Contact */}
          <div>
            <div className="co-form-section-title"><i className="ri-user-line" />Contact Information</div>
            <div className="co-form-grid">
              <Field id="fullName" label="Full Name"     placeholder="Arjun Sharma"    half />
              <Field id="phone"    label="Phone Number"  placeholder="9XXXXXXXXX"      half type="tel" />
              <Field id="email"    label="Email Address" placeholder="you@email.com"    />
            </div>
          </div>
          {/* Address */}
          <div>
            <div className="co-form-section-title" style={{marginTop:8}}><i className="ri-map-pin-line" />Shipping Address</div>
            <div style={{marginBottom:14}}>
              <button className={`co-location-btn${locLoading?" loading":""}`} onClick={detectLocation}>
                <i className={locLoading?"ri-loader-4-line co-spin":"ri-map-pin-2-line"} />
                {locLoading ? "Detecting location…" : "Auto-detect my location"}
              </button>
            </div>
            <div className="co-form-grid">
              <Field id="address" label="Street Address" placeholder="House no, Street, Area" />
              <Field id="city"    label="City"           placeholder="Bhubaneswar" half />
              <Field id="state"   label="State"          as="select"               half />
              <Field id="pincode" label="PIN Code"       placeholder="751001"      half type="tel" />
            </div>
          </div>
        </div>
        <div className="co-nav-row">
          <button className="co-prev-btn" onClick={goPrev}><i className="ri-arrow-left-line" />Back</button>
          <button className="co-next-btn" onClick={goNext}>Continue to Payment <i className="ri-arrow-right-line" /></button>
        </div>
      </div>
    );
  };
